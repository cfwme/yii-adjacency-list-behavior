...
class AdjacencyListBehavior extends CActiveRecordBehavior
{
                        ...
                        $criteria->addColumnCondition(
                            array(
                                 $alias . '.' . $db->quoteColumnName(
                                     $this->owner->getTableSchema()->getTable()->primaryKey
                                 ) => $owner->{$this->parentAttribute}
                            )
                        );
                        break;
                    case self::CHILDREN:
                        $criteria->addColumnCondition(
                            array($alias . '.' . $db->quoteColumnName($this->parentAttribute) => $owner->primaryKey)
                        );
                        break;
                }
            }
    ...
    public function attach($owner)
    {
        ...
        $owner->getMetaData()->addRelation(
            self::PARENT,
            array(CActiveRecord::HAS_ONE, get_class($owner), $this->parentAttribute)
        );
        $owner->getMetaData()->addRelation(
            self::CHILDREN,
            array(CActiveRecord::HAS_MANY, get_class($owner), $this->parentAttribute)
        );
    }
